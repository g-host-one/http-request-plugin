...
public class HttpRequestExecution extends MasterToSlaveCallable<ResponseContentSupplier, IOException> {
    ...
    private HttpRequestExecution(
		...
		if (httpProxy != null && !httpProxy.isBlank()) {
			...
			if (proxyAuthentication != null && !proxyAuthentication.isBlank()) {
                ...
                if (credential instanceof StandardUsernamePasswordCredentials standardUsernamePasswordCredentials) {
        ...
        if (authentication != null && !authentication.isEmpty()) {
            ...
            if (auth == null) {
                ...
                if (credential != null) {
                    ...
                    if (credential instanceof StandardUsernamePasswordCredentials standardUsernamePasswordCredentials) {
                        if (this.useNtlm) {
                            auth = new CredentialNtlmAuthentication(standardUsernamePasswordCredentials);
                        } else {
                            auth = new CredentialBasicAuthentication(standardUsernamePasswordCredentials);
                        }
                    }
                    if (credential instanceof StandardCertificateCredentials standardCertificateCredentials) {
                        auth = new CertificateAuthentication(standardCertificateCredentials, this.ignoreSslErrors);
                    }
                }
            }
    ...
    private ResponseContentSupplier authAndRequest()
        ...
        try {
            ...
            HttpUriRequestBase httpRequestBase = clientUtil.createRequestBase(new RequestAction(new URL(url), httpMode, body, null, headers));
            ...
            if (formData != null && !formData.isEmpty() && httpMode == HttpMode.POST) {
                ...
                httpRequestBase.setEntity(mimeBody);
                ...
                httpRequestBase.setHeader(HttpHeaders.CONTENT_TYPE, entity.getContentType());
    ...
    private CloseableHttpClient auth(
            HttpClientBuilder clientBuilder, HttpUriRequestBase httpRequestBase,
            HttpClientContext context) throws IOException, InterruptedException {

        if (proxyCredentials != null) {
            logger().println("Using proxy authentication: " + proxyCredentials.getId());
            if (authenticator instanceof CredentialBasicAuthentication credentialBasicAuthentication) {
                credentialBasicAuthentication.addCredentials(httpProxy, proxyCredentials);
            } else {
